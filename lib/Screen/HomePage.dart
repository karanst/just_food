...
import 'package:entermarket_user/Helper/AppBtn.dart';
import 'package:entermarket_user/Helper/Color.dart';
import 'package:entermarket_user/Helper/Constant.dart';
import 'package:entermarket_user/Helper/Session.dart';
import 'package:entermarket_user/Helper/SimBtn.dart';
import 'package:entermarket_user/Helper/String.dart';
import 'package:entermarket_user/Helper/app_assets.dart';
import 'package:entermarket_user/Model/Model.dart';
import 'package:entermarket_user/Model/Section_Model.dart';
import 'package:entermarket_user/Model/city_model.dart';
import 'package:entermarket_user/Provider/CartProvider.dart';
import 'package:entermarket_user/Provider/CategoryProvider.dart';
import 'package:entermarket_user/Provider/FavoriteProvider.dart';
import 'package:entermarket_user/Provider/HomeProvider.dart';
import 'package:entermarket_user/Provider/SettingProvider.dart';
import 'package:entermarket_user/Provider/UserProvider.dart';
import 'package:entermarket_user/Screen/SellerList.dart';
...
import 'package:entermarket_user/Screen/starting_view/feature_product.dart';
import 'package:entermarket_user/Screen/starting_view/seller_list.dart';
  ...
  Future<void> getCurrentLoc() async {
   ...
   var loc = Provider.of<LocationProvider>(context, listen: false);
    ...
    if (mounted) {
      setState(() {
        pinController.text = placemark[0].postalCode!;
        currentAddress.text = "${placemark[0].subLocality} , ${placemark[0].locality}";
        latitudeHome = position.latitude.toString();
        longitudeHome = position.longitude.toString();
                ...
                child: SingleChildScrollView(
                  child: Column(
                    ...
                    children: [
                            ...
                            Container(
                              ...
                              child: Row(
                                children: [
                                  Text(
                                    'Categories',
                                    style: TextStyle(fontWeight: FontWeight.w600,color: colors.primary,fontSize: 20),
                                  ),
                                  Spacer(),
                                 /* Text(
                                    "View All",
                      ...
                      SizedBox(height: 10,),
                      _seller(),
                      pages2.length>0?_slider(pages2,1,_controller2):SizedBox(),
                    ],
                  ),
                ),
              )
                  ...
                  itemBuilder: (context, index) {
                      return Padding(
                        ...
                        child: GestureDetector(
                          onTap: () async {
                    //         if(catList[index].name.toString().toLowerCase().contains("pharmacy")){
                    //           Navigator.push(
                    //               context,
                    //               MaterialPageRoute(
  ...
  void getSetting() {
    ...
    apiBaseHelper.postAPICall(getSettingApi, parameter).then((getdata) async {
      ...
      if (!error) {
        ...
        GST_SERVICE_CHARGES = data["order_gst"];
    ...
    if (_isNetworkAvail) {
      try {
        ...
        if(response.body.contains('ï»¿')) {
          temp = response.body.replaceAll('ï»¿', '');
        }

        var getdata = json.decode(temp ?? response.body);
        bool error = getdata["error"];
        String? msg = getdata["message"];
        if (!error) {
          var data = getdata["data"];
...
setState(() {
  ...
  /*void getCity() {
    ...
    apiBaseHelper.postAPICall(getCityApi, map).then((getdata) {
      ...
      if (!error) {
        ...
        for(var v in data){
          setState(() {
            cityList.add(new CityModel(v['id'], v['name'], v['city_id'], v['zipcode_id'], v['minimum_free_delivery_order_amount'], v['delivery_charges']));
          });
        }
        /*setState(() {
  ...
  void getSeller() {
    ...
    print("this is @@ $parameter and ** $getSellerApi");
    ...
    apiBaseHelper.postAPICall(getSellerApi, parameter).then((getdata) {
      ...
      if (!error) {
        ...
        // sellerDetails = data ;
        print("@@ ${sellerList[0].openCloseStatus}");
      } else {
        setSnackbar(msg!, context);
      }
                              ...
                              Navigator.push(
                                  ...
                                  MaterialPageRoute(
                                      builder: (context) => SubCategory(
                                        ...
                                        catId: sellerList[index].category_ids!.contains(",")
                                            ?sellerList[index].category_ids!.split(",")[0]
                                            :sellerList[index].category_ids!,
                                      )));
                            },
                            child: Column(
                              ...
                              children: [
                                Stack(
                                  children: [
                                    sellerList[index].openCloseStatus == 0?
                                    //     sellerDetails[0]['open_close_status'] == 0 ?
                                    Positioned(
                                      top: 10,
                                      left: 20,
                                      child: Text("Closed",
                                      ...
                                      child: Container(
                                        child: Row(
                                          children: [
                                            ...
                                            //   ],
                                            // ),
                                            sellerList[index].openCloseStatus == 1 ?
                                            Text("Closed",
                                              style: TextStyle(
                                                  fontSize: 20,
                                                  color: Colors.red,

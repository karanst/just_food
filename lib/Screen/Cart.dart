...
class StateCart extends State<Cart> with TickerProviderStateMixin {
  ...
  Widget cartItem(int index, List<SectionModel> cartList) {
    ...
    // var gstPrice = (double.parse(cartList[index].perItemTotal!) * double.parse(GST_SERVICE_CHARGES!)) / 100 ;
      ...
      try {
        ...
        apiBaseHelper.postAPICall(getCartApi, parameter).then((getdata) async {
          ...
          if (!error) {
            ...
            if(getdata["data"][0]["product_details"][0]["sellerlatitude"] != null
                && getdata["data"][0]["product_details"][0]["sellerlongitude"] != null){
              itemLat = double.parse(getdata["data"][0]["product_details"][0]["sellerlatitude"].toString());
              itemLong = double.parse(getdata["data"][0]["product_details"][0]["sellerlongitude"].toString());
              print("LATITUDE+++" + itemLat.toString());
              print("LONGITUDE+++" + itemLong.toString());

              if (itemLong != 0.0 && itemLat != 0.0) {
                final double km = distance.as(
                    LengthUnit.Kilometer,
                    new LatLng(itemLat, itemLong),
                    new LatLng(double.parse(loc.lat), double.parse(loc.lng)));
  ...
  razorpayPayment() async {
    ...
    if (contact != '' && email != '') {
      ...
      print("razorpay razorpayId; $razorpayId");
      try {
    ...
    if (_isNetworkAvail) {
      try {
        ...
        if (response.statusCode == 200) {
          ...
          if (!error) {
            var data = getdata["data"];
            promoAmt = double.parse(data[0]["final_discount"]);
            totalPrice = totalPrice + delCharge - promoAmt;
                // double.parse(data[0]["final_total"]) + delCharge;

            promocode = data[0]["promo_code"];
            isPromoValid = true;
            context.read<CartProvider>().setProgress(false);
            setSnackbar(
            ...
            // totalPrice = double.parse(data[0]["final_total"]) + delCharge;
            totalPrice = totalPrice + delCharge - promoAmt;
            context.read<CartProvider>().setProgress(false);
            setSnackbar(msg!, _checkscaffoldKey);
          }
  ...
  Future<double> deliveryChargeApi(km) async {
    ...
    if (_isNetworkAvail) {
      try {
        ...
        //     .timeout(Duration(seconds: timeOut));
        // var getdata = json.decode(response.body);
        apiBaseHelper.postAPICall(deliveryCharge, parameter).then((getdata) {
          bool error = getdata["error"];
          String? msg = getdata["message"];
          var data = getdata["data"];
          context.read<CartProvider>().setProgress(false);
          ...
          if (!error) {
            ...
            for (var i = 0; i < charge.length; i++) {
              if (km >= double.parse(charge[i]["min_km"]) &&
                ...
                print("NO DELIVERY CHARGE APPLY");
              }
            }
          } else {}
        });
